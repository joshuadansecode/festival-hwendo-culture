  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <div className="lg:pl-72">
        ...
        <main className="min-w-0 space-y-6 p-3 sm:p-6 lg:p-8">
...
const Overview = ({ ctx, onNavigate }: any) => {
  ...
  return <><div className="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">{stats.map(([label, value, Icon, target]: any) => <button key={label} onClick={() => onNavigate(target)} className={`${cardClass} min-w-0 p-3 text-left transition hover:-translate-y-1 hover:border-amber-500/50 sm:p-5`}><Icon className="mb-3 h-5 w-5 text-amber-400" /><p className="truncate text-xl font-black sm:text-2xl">{value}</p><p className="mt-1 text-[10px] leading-tight text-gray-400 sm:text-xs">{label}</p></button>)}</div><div className="grid gap-6 xl:grid-cols-2"><section className={`${cardClass} p-4 sm:p-5`}><h3 className="mb-4 font-black">État de la campagne</h3><div className={`rounded-xl p-4 ${ctx.votingConfig.isVotingOpen ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}><b>{ctx.votingConfig.isVotingOpen ? 'Votes ouverts' : 'Votes fermés'}</b><p className="mt-1 text-xs">{ctx.votingConfig.pricePerVoteFCFA} FCFA par vote • minimum {ctx.votingConfig.minVotesPerPurchase}</p></div></section><section className={`${cardClass} p-4 sm:p-5`}><h3 className="mb-4 font-black">Activité récente</h3><div className="space-y-3">{ctx.adminActivities.slice(0, 5).map((a: any) => <div key={a.id} className="flex flex-col gap-1 border-b border-neutral-800 pb-2 text-xs sm:flex-row sm:justify-between"><span><b>{a.action}</b> • {a.entityType}</span><time className="text-gray-500">{new Date(a.createdAt).toLocaleString('fr-FR')}</time></div>)}{!ctx.adminActivities.length && <p className="text-xs text-gray-500">Le journal sera disponible après la migration SQL.</p>}</div></section></div></>;
};
...
const VotingSection = ({ config, save }: any) => { const [data, setData] = useState({ ...config }); return <section className={`${cardClass} w-full max-w-3xl space-y-5 p-4 sm:p-6`}><div className="rounded-xl bg-amber-500/10 p-4 text-xs text-amber-200"><b>Important :</b> cette configuration ne valide aucun paiement. Les votes réels devront être crédités uniquement par webhook serveur.</div><div className="grid gap-4 sm:grid-cols-2"><Field label="Prix par vote"><input className={inputClass} type="number" value={data.pricePerVoteFCFA} onChange={(e) => setData({ ...data, pricePerVoteFCFA: Number(e.target.value) })} /></Field><Field label="Minimum par achat"><input className={inputClass} type="number" value={data.minVotesPerPurchase} onChange={(e) => setData({ ...data, minVotesPerPurchase: Number(e.target.value) })} /></Field><Field label="Début"><input className={inputClass} type="datetime-local" value={data.startDate?.slice(0, 16)} onChange={(e) => setData({ ...data, startDate: e.target.value })} /></Field><Field label="Fin"><input className={inputClass} type="datetime-local" value={data.endDate?.slice(0, 16)} onChange={(e) => setData({ ...data, endDate: e.target.value })} /></Field></div><label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={data.isVotingOpen} onChange={(e) => setData({ ...data, isVotingOpen: e.target.checked })} />Votes ouverts</label><label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={data.showLeaderboardPublicly} onChange={(e) => setData({ ...data, showLeaderboardPublicly: e.target.checked })} />Classement public</label><button onClick={() => save(data)} className="w-full rounded-xl bg-amber-500 px-5 py-3 text-xs font-black text-black sm:w-auto">Enregistrer la campagne</button></section>; };
...
const EditorModal = ({ editor, setEditor, save, upload, uploading, saving }: any) => {
  const data = editor.data;
  const set = (key: string, value: any) => setEditor({ ...editor, data: { ...data, [key]: value } });
  const imageField = editor.type === 'partner' ? 'logo' : editor.type === 'media' ? 'url' : editor.type === 'news' ? 'image' : editor.type === 'participant' || editor.type === 'committee' ? 'photo' : null;
  const labels: Record<string, string> = {
    name: 'Nom complet', number: 'Numéro', category: 'Catégorie', country: 'Pays', community: 'Communauté',
    biography: 'Biographie', projectDescription: 'Projet culturel', photo: 'Photo', voteActive: 'Votes actifs',
    title: 'Titre', subtitle: 'Sous-titre', description: 'Description', date: 'Date', time: 'Heure',
    location: 'Lieu', status: 'Statut', slug: 'Adresse URL', summary: 'Résumé', content: 'Contenu complet',
    author: 'Auteur', featured: 'Mettre en avant', type: 'Type de média', edition: 'Édition / album',
    eventCategory: 'Événement', displayOrder: "Ordre d'affichage", role: 'Fonction', whatsapp: 'WhatsApp',
    facebook: 'Facebook', logo: 'Logo', websiteUrl: 'Site web',
  };
  const options: Record<string, string[]> = {
    category: editor.type === 'participant' ? ['miss-endo', 'top-model', 'styliste'] : editor.type === 'partner' ? ['Sponsor Officiel', 'Média Partner', 'Institution', 'Partenaire Technique'] : ['bureau', 'commission', 'jury'],
    status: ['à venir', 'en cours', 'terminé'], type: ['image', 'video'],
    eventCategory: ['general', 'miss-endo', 'nuit-elegance', 'match-gala'],
  };
  const fields = Object.entries(data).filter(([key]) => !['id', 'votesCount', 'socials', 'thumbnailUrl'].includes(key));
  return <div className="fixed inset-0 z-[110] overflow-y-auto bg-black/80 p-4 backdrop-blur">
    <div className="mx-auto my-8 max-w-3xl rounded-2xl border border-neutral-700 bg-neutral-900 p-5 text-white">
      <div className="mb-5 flex items-center justify-between border-b border-neutral-800 pb-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-amber-400">Formulaire administrateur</p><h2 className="mt-1 text-xl font-black">{data.id ? 'Modifier' : 'Ajouter'} • {editor.type === 'participant' ? 'Participant' : editor.type}</h2></div><button onClick={() => setEditor(null)} aria-label="Fermer"><X /></button></div>
      {editor.type === 'participant' && <div className="mb-5 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-100">Les votes sont crédités uniquement après confirmation LeekPay. Le compteur ne peut pas être modifié depuis ce formulaire.</div>}
      <div className="grid gap-4 sm:grid-cols-2">
        {fields.map(([key, value]) => {
          const label = labels[key] ?? key;
          if (key === imageField) return <Field key={key} label={label}><div className="space-y-2"><input className={inputClass} value={String(value ?? '')} onChange={(e) => set(key, e.target.value)} placeholder="URL publique ou téléverser un fichier" /><label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-neutral-600 p-3 text-xs font-bold text-amber-400 hover:border-amber-500"><Upload className="h-4 w-4" />{uploading ? 'Téléversement...' : 'Choisir un fichier'}<input hidden type="file" accept="image/*" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0], editor.type, key)} /></label>{String(value) && <img src={String(value)} alt="Aperçu" className="h-24 w-full rounded-lg object-cover" />}</div></Field>;
          if (typeof value === 'boolean') return <label key={key} className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={value} onChange={(e) => set(key, e.target.checked)} />{label}</label>;
          if (options[key]) return <Field key={key} label={label}><select className={inputClass} value={String(value ?? '')} onChange={(e) => set(key, e.target.value)}>{options[key].map((option) => <option key={option} value={option}>{option}</option>)}</select></Field>;
          if (['biography', 'projectDescription', 'summary', 'content', 'description'].includes(key)) return <Field key={key} label={label}><textarea className={`${inputClass} min-h-24`} value={String(value ?? '')} onChange={(e) => set(key, e.target.value)} placeholder={`Saisir ${label.toLowerCase()}...`} /></Field>;
          return <Field key={key} label={label}><input className={inputClass} value={String(value ?? '')} onChange={(e) => set(key, key === 'displayOrder' ? Number(e.target.value) : e.target.value)} placeholder={`Saisir ${label.toLowerCase()}...`} /></Field>;
        })}
      </div>
      <div className="mt-6 flex justify-end gap-3 border-t border-neutral-800 pt-4"><button onClick={() => setEditor(null)} className="rounded-xl border border-neutral-700 px-4 py-2 text-xs font-bold">Annuler</button><button onClick={save} disabled={saving || uploading} className="rounded-xl bg-amber-500 px-5 py-2 text-xs font-black text-black disabled:opacity-50">{saving ? 'Enregistrement...' : 'Enregistrer'}</button></div>
    </div>
  </div>;
};
